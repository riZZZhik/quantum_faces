...
class ImagePreparation:
    ...
    def get_paths_to_images(images_path):
        ...
        if type(images_path) == str:
            ...
            images_path = glob(images_path + "/*.jpg") + glob('quantum_faces/faces/*.jpeg') + glob(
                images_path + "/*.png")
        else:
    ...
    def norm_images_from_disk(self, images_path, crop_type, resize_size=(32, 32)):
        ...
        if crop_type == 0:
            norm_images = [self.image_normalization(image, *resize_size) for image in images]
        else:
            for image in images:
                            ...
                            if coordinates:
                                ...
                                for x, y in landmarks[-1]:
                                    if shape[0] > y >= 0 and shape[1] > x >= 0:
                                        landmarks_images[-1] = cv2.circle(landmarks_images[-1], (x, y), size, 255, -1)

                                landmarks_images[-1] = Image.fromarray(landmarks_images[-1], "L")
                                norm_images.append(self.image_normalization(landmarks_images[-1], *resize_size))
                    else:
                        norm_images.append(self.image_normalization(face_images[-1], *resize_size))
